...
if($help==1){
	...
	[psidb]: location of BLAST database intended for PSSM contruction. Suggested that one use a clustered database (i.e. Uniref50).\n
	...
	Mode Selection:\n
	[ds]: Domain specific toggle. You will only use the matched region of a subject sequence for future searches.\n
			This option filters out matches that are 20% larger or smaller than your query.\n
...
if("$psidatabase.pto" and "$psidatabase.ptf" and "$psidatabase.pot" and "$psidatabase.pos" and "$psidatabase.pdb" and "$psidatabase.pal"){}
else{
