...
#NOTES FOR SELF:
#code needs to remove seed sequences from home directory between cycles.
#output is also not working as intended
...
sub PSIBLAST{
	...
	my $dir = shift;
	my @query_files = @_;
	#add line to export BLASTDB path to system i.e. BLASTDB=/isg/shared/databases/uniprot/uniref50
	foreach my $seq (@query_files){
  	...
  	close RANGE;
		close PSI;
  	system("blastdbcmd -db $extractdb -entry_batch range_file.txt -outfmt \"%f\" > all_matches.$database");
		system("mv $seq.$extension $seq.pssm $seq.asci.pssm range_file.txt $seq temp.txt $dir");
	}
	return("all_matches.$database");
}
...
sub RE_SEED{
	...
	foreach my $seedseq (@cluster_seeds){
		system("cp $seedseq archive/$seedseq");
	}
